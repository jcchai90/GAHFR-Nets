...
def load_checkpoint(
    ...
    if resume_loc is not None and os.path.isfile(resume_loc):
        ...
        required_keys = ["epoch", "iterations", "best_metric", "model_state_dict", "optim_state_dict", "gradient_scalar_state_dict"]
        has_all_keys = all(key in checkpoint for key in required_keys)

        if has_all_keys:
            # 完整的checkpoint，恢复所有状态
            ...
            if is_master_node:
                logger.log("Loaded full checkpoint from {}".format(resume_loc))
                logger.log("Resuming training for epoch {}".format(start_epoch))
        else:
            # 只有模型权重的checkpoint，只加载模型，不恢复训练状态
            if is_master_node:
                logger.log("Checkpoint only contains model weights, not full training state")
                logger.log("Loading model weights only, starting training from epoch 0")
            
            # 尝试直接加载（如果checkpoint本身就是state_dict）
            if "model_state_dict" in checkpoint:
                model = load_state_dict(model, checkpoint["model_state_dict"])
            else:
                # 或者checkpoint本身就是state_dict
                model = load_state_dict(model, checkpoint)
    else:
        if is_master_node:
